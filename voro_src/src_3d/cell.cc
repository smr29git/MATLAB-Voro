...
namespace voro {
  ...
  /** Calculates the transport cost of the Voronoi cell, by decomposing the cell into
   * tetrahedra extending outward from the centroid, whose volumes are
   * evaluated using a scalar triple product.
   * \return A floating point number holding the calculated volume. */
  double voronoicell_base::transportcost() {
    double cost=0;
    int i,j,k,l,m,n;
    double ux,uy,uz,vx,vy,vz,wx,wy,wz;
	...
	if(k>=0) {
	  ...
	  while(m!=i) {
	    ...
	    double det=ux*vy*wz+uy*vz*wx+uz*vx*wy-uz*vy*wx-uy*vx*wz-ux*vz*wy;
	    double tr=pow((ux+vx+wx),2.)+ux*ux+vx*vx+wx*wx+pow((uy+vy+wy),2.)+uy*uy+vy*vy+wy*wy+pow((uz+vz+wz),2.)+uz*uz+vz*vz+wz*wz;

	    cost+=-det*tr/(32.*120.);
	    //	    printf("cost so far: %g\n",cost);
	    k=m;l=n;
	    m=ed[k][l];ed[k][l]=-1-m;
	  }
	}
