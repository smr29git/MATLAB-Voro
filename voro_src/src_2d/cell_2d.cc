...
namespace voro {
  ...
  double voronoicell_base_2d::area() {
    ...
    if(p==0) return 0;
    int k=0,l;double area=0,x1,y1,x2,y2;
    x1=pts[2*k];y1=pts[2*k+1];
    do {
      l=ed[2*k];
      x2=pts[2*l];
      y2=pts[2*l+1];
  ...
  double voronoicell_base_2d::transport_cost() {
    if(p==0) return 0;
    int k=0,l;double t=0,x1,y1,x2,y2;
    x1=pts[2*k];y1=pts[2*k+1];
    do {
      l=ed[2*k];
      x2=pts[2*l];
      y2=pts[2*l+1];
      t+=-(2.*x1*x2*(y1+y2)+x1*x1*(3.*y1+y2)+x2*x2*(y1+3.*y2))*(x2-x1);
      t+=(x1*(3.*y1*y1+2.*y1*y2+y2*y2)+x2*(y1*y1+2.*y1*y2+3.*y2*y2))*(y2-y1);
      k=l;x1=x2;y1=y2;
    } while (k!=0);
    return 0.0625*t/12.;
  }

  double voronoicell_base_2d::x2moment(){
    if(p==0) return 0;
    int k=0,l;double x2m=0,x1,y1,x2,y2;
    x1=pts[2*k];y1=pts[2*k+1];
    do {
      l=ed[2*k];
      x2=pts[2*l];
      y2=pts[2*l+1];
      x2m+=(x1*x1+x2*x2)*(x1+x2)*(y2-y1);
      k=l;x1=x2;y1=y2;
    } while (k!=0);
    return 0.0625*x2m/12.;
  }
  ...
  void voronoicell_base_2d::centroid(double &cx,double &cy) {
    ...
    do {
      ...
      x2=pts[2*l];
      y2=pts[2*l+1];
      cx+=(y2-y1)*(x1*x1+x1*x2+x2*x2);
      cy+=(x1-x2)*(y1*y1+y1*y2+y2*y2);
      area+=0.5*(x1+x2)*(y2-y1);
      k=l;x1=x2;y1=y2;
    }while(k!=0);
    //    cx=0.5*(x+cx*tarea);
    //    cy=0.5*(y+cy*tarea);
    cx=0.5*cx*sixth/area;
    cy=0.5*cy*sixth/area;
  }
