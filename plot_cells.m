function [h,id]=plot_cells(vfn,varargin)

% h=plotGrains(vfn,OPTid,OPTcellcolour);
% 
% Plot individual grains
%   input aguments
%
%   vfn      : vertex, face and neighbour data
...
% Obtain the number of cells
    ...
    % If there are additional arguments they should refer to the colour of the faces of the cell
    if(nargin>3)
    ...
    for j=1:nid,
        ...
        if(~isempty(vfn{cid,1}))
            ...
            % For the cell, get the number of faces
            [Nf,~]=size(faces);
            ...
            for k=1:Nf,
                fc=faces{k};
                fv=verts(fc,:);
                %            h{j}(i)=patchPolygon3d(fv);
                h{j}(k)=fill3(fv(:,1),fv(:,2),fv(:,3),cellcolour(j,:));
                %set(h{j}(k),'facecolor',graincolour(j,:));
            end
            hold off
